...
indir_bkg = "root://cmseos.fnal.gov//store/user/xshen/BtoTW_Jul2023/Run2018_Jul2023/"
indir_sig = "root://cmseos.fnal.gov//store/user/kjohnso/BtoTW_Jul2023/Run2018_Jul2023/"
outdir = os.getcwd()+'/plots_bkgsig/'
if not os.path.exists(outdir): os.system('mkdir -p '+outdir)

samples_sig = {'Bp800':'Bprime_M800__20UL18/RDF_BprimeBtoTW_M-800_NWALO_TuneCP5_13TeV-madgraph-pythia8_finalsel_0.root',
...
samples_bkg = {#'DYMHT1200':'DYMHT12002018UL_hadd.root',
...
if(getHistos):
    ...
    for sample in samples_sig:
        ...
        for branch in branches:
            ...
            histo_weighted = histfile.Get(branch + "_" + sample)
            histo_weighted.Scale(weight)
        
            histfile.cd()
            histo_weighted.Write(branch+"_"+sample+"_weighted")

    for sample in samples_bkg:
        filename = indir_bkg+samples_bkg[sample]
        Events = RDataFrame("Events", filename).Define("weight","Generator_weight*{}*{}/({}*abs(Generator_weight))".format(lumi,xsec[sample],nRun[sample]))
        infile = TFile.Open(filename, "READ")
        ftree = infile.Get("Events")
        ftree.GetEntry(0)
        Generator_weight = ftree.Generator_weight
...
for branch in branches:
    ...
    for sig in samples_sig:
        ...
        histo.SetLineColor(colors[sig])
        histo.SetFillColor(colors[sig])
    ...
    for bkg in background_list:
        ...
        histo.SetLineColor(colors[bkg])
        histo.SetFillColor(colors[bkg])
        #histo.SetFillStyle(3002)
    ...
    histo_stack_bkg.Draw("HIST SAME")
    Legend.Draw()
    c1.Update()
